...
function renderBooks(booksToRender) {
  const booksGrid = document.getElementById("booksGrid");
  const gridHeight = booksGrid.clientHeight || window.innerHeight;
  const bookCardHeight = 400;
  const visibleRows = Math.ceil(gridHeight / bookCardHeight);
  const cardsPerRow = Math.floor(
    (booksGrid.clientWidth || window.innerWidth) / 280
  );

  virtualizedBooks = booksToRender;
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (
          ...
          virtualizedBooks.length > (currentPage + 1) * BOOKS_PER_PAGE
        ) {
          observer.unobserve(entry.target);
          currentPage++;
          requestAnimationFrame(() => {
            renderBooks(virtualizedBooks);
...
const cleanupUnusedBooks = debounce(() => {
  const visibleBooks = document.querySelectorAll(".book-card");
  let removedCount = 0;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("scroll", cleanupUnusedBooks, { passive: true });
  window.addEventListener(
    "resize",
    debounce(() => renderBooks(virtualizedBooks), 150),
    { passive: true }
  );
});
